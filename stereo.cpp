							  ...
							  int WinRadL0, float MaxScaler, bool USE_SAD)
{
	...
	if(!USE_SAD)
	{
		...
		for(d=0;d<m_Depth;d++)
		{
			...
			// For each pixel in the reference image...
			for(c0=0;c0<m_Cols;c0++) // problem: imageSums.getValue() is called with a column value (c1) that is out of range... but, when i wrap an if-statement (to prevent these bad accesses) around the innermost for-loop below, the image output is all black... i'm not sure where the bug is... so, i've simply disabled normalized correlation in run-zks.cpp because the SAD method works fine.
				for(r0=0;r0<m_Rows;r0++)
				{
					// Compute the disparity in pixels
					disparityDel = d + m_MinD;
...
void CStereo::PrintDisparityValues(const char *name)
{
	int r,c;
	FILE *outFile;
...
void CImage::sumImage(int radius)
{
	...
	for(i=0;i<m_color;i++)
	{
		...
		for(col=radius;col<m_width-radius;col++)
		{
			...
			for(row=radius;row<m_height-radius-1;row++)
			{
				tmpa[row] = sum;
				sum -= *pt0;
				sum += *pt1;
			...
			for(row=radius;row<m_height-radius;row++, pt0 += offset)
				*pt0 = tmpa[row];
		}

		/* sum along rows */
		offset = m_color;
		for(row=radius;row<m_height-radius;row++)
		{
			sum = 0.0;
			for(col=radius-radius;col<=radius+radius;col++)
				sum += getValue(row, col, i);
